...
// Orange Theme with White Text Palette
const C_BG = 'C2410C';        // Deep Vibrant Industrial Orange Background
const C_CARD_BG = '6B2006';   // Dark Burnt Orange Card Background
const C_CARD_HIGHLIGHT = '872808';// Rich Warm Card Highlight
const C_WHITE = 'FFFFFF';     // Crisp White Text
const C_WHITE_SOFT = 'FFF7ED';// Soft Cream White
const C_YELLOW = 'FDE047';    // Bright Warm Sun Yellow Accent
const C_CYAN = '67E8F9';      // Cyan Accent
const C_GREEN = '4ADE80';     // Bright Green Accent
const C_MUTED = 'FED7AA';     // Light Amber Muted Text
const C_BORDER = 'E06528';    // Translucent Border
const C_BORDER_WHITE = 'FFFFFF';// White Border Accent
const C_CODE_BG = '140702';   // Code Terminal Dark Background

function addSlideHeader(slide, title, category, slideNum) {
    slide.background = { color: C_BG };

    // Top White Gradient Accent Bar
    slide.addShape(pptx.shapes.RECTANGLE, {
        x: 0, y: 0, w: 13.333, h: 0.1,
        fill: { color: C_WHITE }, line: { color: C_WHITE }
    });

    // Category Badge
    slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        x: 0.8, y: 0.35, w: 3.8, h: 0.34,
        fill: { color: '451203' },
        line: { color: C_WHITE, width: 1.2 },
        rectRadius: 0.08
    });
    slide.addText(`⚙  ${category}`, {
        x: 0.8, y: 0.35, w: 3.8, h: 0.34,
        fontSize: 9.5, color: C_YELLOW, bold: true, align: 'center', valign: 'middle'
    });
    ...
    slide.addText('WELLORA™ SCADA Digital Twin | Heavy Crude Oil (17.5° API) AI Optimization System', {
        ...
        fontSize: 9, color: C_MUTED, fontFace: 'Segoe UI'
    });

    if (slideNum) {
        slide.addText(`Slide ${slideNum} / 24`, {
            x: 10.5, y: 7.15, w: 2.0, h: 0.3,
            fontSize: 9.5, color: C_WHITE, bold: true, align: 'right', fontFace: 'Segoe UI'
        });
    }
}
...
s1.addShape(pptx.shapes.RECTANGLE, { x: 0, y: 0, w: 13.333, h: 0.12, fill: { color: C_WHITE } });
s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 1.0, y: 0.8, w: 11.333, h: 5.8,
    fill: { color: '541804' }, line: { color: C_WHITE, width: 2 }, rectRadius: 0.15
});
s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 1.5, y: 1.2, w: 4.8, h: 0.38,
    fill: { color: '360E02' }, line: { color: C_YELLOW, width: 1.2 }, rectRadius: 0.08
});
...
s1.addText('WELLORA', {
    ...
    fontSize: 48, color: C_WHITE, bold: true, fontFace: 'Segoe UI'
});
s1.addText('Dual-Physics Digital Twin & Real-Time SCADA AI Optimization System for Heavy Crude (17.5° API)', {
    ...
    fontSize: 18, color: C_WHITE_SOFT, bold: true, fontFace: 'Segoe UI'
});
s1.addText('Coupled Andrade Thermal Viscosity Modeling, Sucker Rod Pump (SRP) Kinematics, Cyclic Steam Stimulation (CSS) Enhanced Oil Recovery, Live SCADA Oscilloscope & Conversational AI Copilot', {
    ...
    fontSize: 12.5, color: C_MUTED, fontFace: 'Segoe UI'
});
...
bgs.forEach((b, idx) => {
    s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: '3B1002' }, line: { color: C_WHITE, width: 1 }, rectRadius: 0.06
    });
    s1.addText(b, {
        x: 1.5 + (idx * 2.05), y: 4.45, w: 1.95, h: 0.35,
        fontSize: 8.5, color: C_WHITE, bold: true, align: 'center', valign: 'middle'
    });
});
...
s1.addText('Target Reservoir: 17.5° API Crude Oil (3,500 cP Viscosity)\nDeployment: Vercel Production Cloud / Zero-Config', {
    ...
    fontSize: 10, color: C_MUTED, align: 'right', fontFace: 'Segoe UI'
});

// Helper for 2-column text slide with Orange background and White text
function addTwoColSlide(title, cat, num, leftTitle, leftBullets, rightTitle, rightBullets, highlightRight = false) {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: C_CARD_BG }, line: { color: C_BORDER, width: 1.2 }, rectRadius: 0.1
    });
    s.addText(leftTitle, {
        x: 1.1, y: 1.8, w: 5.1, h: 0.35, fontSize: 12, color: C_YELLOW, bold: true
    });
    let yL = 2.25;
    leftBullets.forEach(b => {
        s.addText(`• ${b}`, {
            x: 1.1, y: yL, w: 5.1, h: 0.9, fontSize: 10, color: C_WHITE_SOFT, fontFace: 'Segoe UI'
        });
        yL += 0.95;
    });
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: highlightRight ? C_CARD_HIGHLIGHT : C_CARD_BG },
        line: { color: highlightRight ? C_WHITE : C_BORDER, width: 1.2 },
        rectRadius: 0.1
    });
    s.addText(rightTitle, {
        x: 7.1, y: 1.8, w: 5.1, h: 0.35, fontSize: 12, color: highlightRight ? C_WHITE : C_YELLOW, bold: true
    });
    let yR = 2.25;
    rightBullets.forEach(b => {
        s.addText(`• ${b}`, {
            x: 7.1, y: yR, w: 5.1, h: 0.9, fontSize: 10, color: C_WHITE_SOFT, fontFace: 'Segoe UI'
        });
        yR += 0.95;
    });
}
...
function addImageSlide(title, cat, num, imgPath, descTitle, bullets) {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: '2B0901' }, line: { color: C_WHITE, width: 1.5 }, rectRadius: 0.1
    });
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: C_CARD_HIGHLIGHT }, line: { color: C_WHITE, width: 1.2 }, rectRadius: 0.1
    });
    s.addText(descTitle, {
        x: 8.8, y: 1.8, w: 3.5, h: 0.35, fontSize: 11.5, color: C_YELLOW, bold: true
    });
    let yP = 2.25;
    bullets.forEach(b => {
        s.addText(`• ${b}`, {
            x: 8.8, y: yP, w: 3.5, h: 0.95, fontSize: 9.5, color: C_WHITE_SOFT, fontFace: 'Segoe UI'
        });
        yP += 1.0;
    });
}
...
function addCodeSlide(title, cat, num, codeHeader, codeText, descTitle, bullets) {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        x: 0.8, y: 1.6, w: 7.6, h: 5.2,
        fill: { color: C_CODE_BG }, line: { color: C_WHITE, width: 1.2 }, rectRadius: 0.1
    });
    s.addText(codeHeader, {
        x: 1.0, y: 1.75, w: 7.2, h: 0.25,
        fontSize: 9, color: C_YELLOW, fontFace: 'Consolas'
    });
    s.addText(codeText, {
        x: 1.0, y: 2.05, w: 7.2, h: 4.6,
        fontSize: 8.0, color: 'FFFFFF', fontFace: 'Consolas'
    });
...
addTwoColSlide(
    ...
    'Industrial Orange Palette & Tokens',
    [
        'High-Contrast Industrial Theme: Deep vibrant orange background (#C2410C) with pure white text (#FFFFFF) for 24/7 control room readability.',
        'Status Semantic Colors: Green (#4ADE80) for Normal, Yellow (#FDE047) for Warning, Red (#F87171) for Alarms, Cyan (#67E8F9) for Steam Injection.',
        'Typography: Space Grotesk / Plus Jakarta Sans for headings + JetBrains Mono for telemetry numerals.'
    ],
...
addCodeSlide(
    ...
    '    return {\n' +
    '      text: "⚡ Dynamic Simulation for " + targetSpm + " SPM: Peak Polish Rod Load calculates to " + sim.pprl.toLocaleString() + " lbs with " + sim.motorHPEstimate + " HP demand.",\n' +
...
const outputFile = path.join(__dirname, 'WELLORA_Project_Presentation_OrangeTheme.pptx');
console.log('Writing 24-slide Orange Theme PPTX file:', outputFile);

pptx.writeFile({ fileName: outputFile })
    .then(f => {
        console.log('SUCCESS: Orange Theme PowerPoint presentation created at:', f);
        try {
            ...
            console.log('Note: WELLORA_Project_Presentation.pptx is open; saved WELLORA_Project_Presentation_OrangeTheme.pptx.');
        }
    })
    .catch(err => {

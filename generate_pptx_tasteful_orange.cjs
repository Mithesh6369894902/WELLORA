...
// Clean Dark Slate Background with Tasteful Orange Accents
const C_BG = '0B0F19';        // Sleek Dark Slate
const C_CARD_BG = '121826';   // Clean Container Card
const C_CARD_HIGHLIGHT = '172033';// Highlight Card
const C_ORANGE = 'FF6B00';    // Tasteful Cyber-Orange Accent
const C_ORANGE_SUBTLE = '2A1708';// Subtle Orange Tint
const C_ORANGE_BORDER = 'E06528';// Orange Border Accent
const C_WHITE = 'FFFFFF';     // Crisp White Heading & Body
const C_WHITE_SOFT = 'F1F5F9';// Soft Light Text
const C_MUTED = '94A3B8';     // Muted Slate Text
const C_YELLOW = 'FDE047';    // Warm Sun Yellow Accent
const C_CYAN = '38BDF8';      // Cyan Accent
const C_GREEN = '34D399';     // Emerald Green
const C_BORDER = '1E293B';    // Subtle Border
const C_CODE_BG = '07090E';   // Deep Dark Code Terminal
...
function addSlideHeader(slide, title, category, slideNum) {
    ...
    slide.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: C_ORANGE_SUBTLE },
        line: { color: C_ORANGE, width: 1.2 },
        rectRadius: 0.08
    });
    slide.addText(`⚙  ${category}`, {
        x: 0.8, y: 0.35, w: 3.8, h: 0.34,
        fontSize: 9.5, color: 'FFA726', bold: true, align: 'center', valign: 'middle'
    });
    ...
    // Divider Line with Orange Highlight Accent
...
s1.addShape(pptx.shapes.RECTANGLE, { x: 0, y: 0, w: 13.333, h: 0.1, fill: { color: C_ORANGE } });
s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 1.0, y: 0.8, w: 11.333, h: 5.8,
    fill: { color: C_CARD_BG }, line: { color: C_ORANGE, width: 1.5 }, rectRadius: 0.15
});
s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
    x: 1.5, y: 1.2, w: 4.8, h: 0.38,
    fill: { color: C_ORANGE_SUBTLE }, line: { color: C_ORANGE, width: 1.2 }, rectRadius: 0.08
});
...
bgs.forEach((b, idx) => {
    s1.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: C_ORANGE_SUBTLE }, line: { color: C_ORANGE, width: 1 }, rectRadius: 0.06
    });
    s1.addText(b, {
        x: 1.5 + (idx * 2.05), y: 4.45, w: 1.95, h: 0.35,
        fontSize: 8.5, color: 'FFA726', bold: true, align: 'center', valign: 'middle'
    });
});
...
// Helper for 2-column text slide with subtle orange accents
function addTwoColSlide(title, cat, num, leftTitle, leftBullets, rightTitle, rightBullets, highlightRight = false) {
    ...
    s.addText(leftTitle, {
        x: 1.1, y: 1.8, w: 5.1, h: 0.35, fontSize: 12, color: 'FF8C38', bold: true
    });
    let yL = 2.25;
    ...
    s.addText(rightTitle, {
        x: 7.1, y: 1.8, w: 5.1, h: 0.35, fontSize: 12, color: highlightRight ? C_WHITE : 'FF8C38', bold: true
    });
    let yR = 2.25;
...
function addImageSlide(title, cat, num, imgPath, descTitle, bullets) {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        ...
        fill: { color: '06090F' }, line: { color: C_ORANGE_BORDER, width: 1.2 }, rectRadius: 0.1
    });
    ...
    s.addText(descTitle, {
        x: 8.8, y: 1.8, w: 3.5, h: 0.35, fontSize: 11.5, color: 'FF8C38', bold: true
    });
    let yP = 2.25;
    bullets.forEach(b => {
        s.addText(`• ${b}`, {
...
function addCodeSlide(title, cat, num, codeHeader, codeText, descTitle, bullets) {
    ...
    s.addShape(pptx.shapes.ROUNDED_RECTANGLE, {
        x: 0.8, y: 1.6, w: 7.6, h: 5.2,
        fill: { color: C_CODE_BG }, line: { color: C_BORDER, width: 1.2 }, rectRadius: 0.1
    });
    s.addText(codeHeader, {
        x: 1.0, y: 1.75, w: 7.2, h: 0.25,
        fontSize: 9, color: 'FFA726', fontFace: 'Consolas'
    });
    s.addText(codeText, {
...
addTwoColSlide(
    ...
    [
        'High-Contrast Theme: Clean dark slate canvas (#0B0F19) with tasteful Cyber-Orange accents (#FF6B00) and crisp white text (#FFFFFF).',
        'Status Semantic Colors: Green (#34D399) for Normal, Amber (#FFA726) for Warning, Red (#F87171) for Alarms, Cyan (#38BDF8) for Steam Injection.',
        'Typography: Space Grotesk for headings + JetBrains Mono for telemetry numerals.'
    ],
    'Reactive Component Tree & State Flow',
    [
        'App.tsx: Central telemetry coordinator managing 2.0 Hz telemetry ticks, active well selection, and simulation parameters.',
